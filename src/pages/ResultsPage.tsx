...
export function ResultsPage() {
  ...
  async function handleRegenerate() {
    ...
    const success = await generateRecipes({ extraPrompt: regeneratePrompt });
    if (success) {
      setRegeneratePrompt("");
    }
  }
    ...
    <DeviceFrame>
      ...
      <header className="mt-6 px-1">
        <h1 className="text-[24px] font-black leading-8 tracking-normal text-[#151411]">
          {hasRecipes ? `为你推荐 ${generatedRecipes.length} 道菜` : "还没有菜谱"}
        </h1>
        <p className="mt-2 text-[12px] font-semibold text-[#9b958d]">
          {hasRecipes ? "基于你的食材、调料和偏好生成。" : "回到首页，选好食材后再生成。"}
        </p>
      </header>
      ...
      {hasRecipes ? (
        ...
        <section className="mt-5 rounded-[18px] border border-[#eeeae4] bg-white p-5 text-center shadow-[0_8px_26px_rgba(27,24,20,0.07)]">
          <p className="text-[14px] font-extrabold text-[#1b1a17]">菜谱还没生成出来</p>
          <p className="mt-2 text-[12px] font-semibold leading-5 text-[#9b958d]">
            先告诉我今晚有什么食材，再让模型帮你凑一顿。
          </p>
          <MotionLink
            className="mt-4 inline-flex h-10 items-center justify-center rounded-full bg-[#111] px-5 text-[13px] font-bold text-white"
            to="/"
            {...motionTapProps}
          >
            回首页
